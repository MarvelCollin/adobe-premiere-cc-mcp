...
export const projectTools = defineTools([
...

  {
    name: "run_script",
    description:
      "Escape hatch: run raw ExtendScript in Premiere for anything the typed tools do not cover. ES3 only (var, no arrow functions, no template literals, no JSON object). End with `return __result({...})` or `return __error('...')`. Helpers: __seq() active sequence; __qe() its QE sequence; __findClip(nodeId) returns a wrapper {clip, trackIndex, clipIndex, trackType}, so use .clip for the TrackItem; __qeTrackFor(wrapper) then __qeClipAt(qeTrack, seconds) for the QE clip, which is where .speed lives; __component(clip, name) and __property(component, name) for effects; also __componentNames, __clearKeys, __ticksToSeconds, __secondsToTicks.",
    schema: {
